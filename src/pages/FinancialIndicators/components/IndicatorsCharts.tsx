...
export const IndicatorsCharts: React.FC<IndicatorsChartsProps> = ({
  ...
  const handlePieClick = async (_event: unknown, elements: { index: number }[]) => {
      ...
      legend: {
        ...
        labels: {
          ...
          generateLabels: function(chart: any) {
            ...
            if (currentLevel.type === 'direct_costs') {
              const labels: { text: string; fillStyle: string; strokeStyle?: string; lineWidth?: number; hidden: boolean; index: number; fontColor: string; fontStyle?: string }[] = [];
        ...
        maxWidth: 200,
        onClick: function(e: unknown, legendItem: { index?: number }) {
          // Игнорируем клики на заголовки и разделители
          if (legendItem.index === undefined || legendItem.index < 0) return;

          // Вызываем handlePieClick программно
          handlePieClick(e, [{ index: legendItem.index }]);
        },
      },
      tooltip: {
        callbacks: {
  ...
  const handleBarClick = async (_event: unknown, elements: { index: number }[]) => {
    if (elements.length === 0) return;
            ...
            >
              ...
              <Spin spinning={loadingBreakdown}>
                <Table
                  ...
                  scroll={{ x: 1200 }}
                  summary={(tableData) => {
                    const data = tableData as readonly CategoryBreakdown[];
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24}>
            ...
            <Table
              ...
              scroll={{ x: 650 }}
              summary={(pageData) => {
                const data = pageData as readonly { amount: number }[];
