 ...
 * Преобразует русский формат (запятая) в формат для JS (точка)
 * Вызывается когда пользователь вводит значение
 *
 * @param value - строковое значение из поля ввода (может содержать запятую)
 * @returns число для внутреннего использования
 *
 * @example
 * <InputNumber parser={parseNumberInput} formatter={formatNumberInput} />
 * // Пользователь вводит "1,5" → parser преобразует в 1.5
 */
export const parseNumberInput = (value: string | undefined): number => {
  if (!value || value.trim() === '') return 0;
  // Заменяем запятую на точку для корректного парсинга JavaScript
 ...
 * Убирает пробелы и заменяет запятую на точку
 *
 * @param value - строка из поля ввода (может содержать пробелы и запятые)
 * @returns число для внутреннего использования
 *
 * @example
